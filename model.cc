...
void linear_model(F_TYPE input[INPUT_SIZE],
    ...
    copy_1d<INPUT_SIZE>(input, input_local);
    // copy weight to local memory
    copy_2d<OUTPUT_SIZE, INPUT_SIZE>(weight, weight_local);

    // linear layer    
    linear<INPUT_SIZE, OUTPUT_SIZE>(input_local, output_local, weight_local);

    // copy output_local to output
    copy_1d<OUTPUT_SIZE>(output_local, output);
